...
import { Equivalencia, EquivalenciaCalculada } from '../models/equivalencia-response';
...
export class EquivalenciaService {
  ...
  listarPorAlimento(alimentoId: number, quantidadeGramas?: number): Observable<EquivalenciaCalculada[]> {
    let params = new HttpParams();
    if (quantidadeGramas !== undefined && quantidadeGramas !== null) {
      params = params.set('quantidadeGramas', String(quantidadeGramas));
    }
    return this.http.get<EquivalenciaCalculada[]>(`${this.apiUrl}/alimento/${alimentoId}`, { params });
  }
